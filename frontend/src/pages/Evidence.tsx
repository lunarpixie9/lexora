...
export default function Evidence() {
  ...
  const body = (
    <div className="space-y-6">
      <div className="card p-6">
        <p className="text-sm text-ink-600">Written for reviewers and examiners. If you are a teacher or parent,{' '}
        ...
        <p className="mt-2 text-sm text-ink-700">Each part of Lexora is checked against a real dataset that fits that part. Passing these checks shows the pipeline behaves sensibly on real data. It does <strong>not</strong> show that Lexora can identify dyslexia — no approved dataset provides dyslexia labels for Indian children’s English, and Lexora does not claim to.</p>
        <ol className="mt-4 grid gap-3 text-sm md:grid-cols-4">
          ...
          ].map(([t, b]) => <li key={t} className="rounded-xl bg-cream-100 p-3"><p className="font-bold">{t}</p><p className="mt-1 text-ink-600">{b}</p></li>)}
        </ol>
      </div>
        ...
        <>
          {v.reading_level_model && (
            <Section title="Reading-level model — ASER (real Indian children, 4,908 sessions)">
              ...
              <p className="mt-3 text-sm text-ink-700"><strong>Top SHAP features:</strong> {v.reading_level_model.shap_mean_abs_by_feature.slice(0, 5).map((s) => `${s.feature} (${s.mean_abs_shap})`).join(', ')}</p>
              <p className="mt-2 text-sm text-ink-700"><strong>Typical level by class (median, real data):</strong> {Object.entries(v.reading_level_model.class_norms).map(([c, n]) => `class ${c}: ${n.median_level}`).join(' · ')}</p>
              <p className="mt-2 text-sm text-ink-700"><strong>Reading-rate reference (correct sentences):</strong> {Object.entries(v.reading_level_model.sentence_wpm_reference_by_class).map(([c, n]) => `class ${c}: ${n.median_wpm} wpm`).join(' · ')}</p>
              <p className="mt-3 rounded-xl bg-sun-100/60 p-3 text-xs text-ink-700">{v.reading_level_model.caveat}</p>
            </Section>
          )}
            ...
            <Section title="Spelling-error engine — Birkbeck & Holbrook corpora (real misspellings)">
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="text-left text-xs uppercase tracking-wider text-ink-600"><tr><th className="py-1 pr-3">Corpus</th><th className="pr-3">Errors</th><th className="pr-3">Detected</th><th className="pr-3">Pattern assigned</th><th className="pr-3">Phonetically plausible</th><th>False positives</th></tr></thead>
                  <tbody>
              ...
              <p className="mt-3 text-xs text-ink-600">{v.nlp_misspellings.note}</p>
            </Section>
          )}
          ...
          {v.speech_aser && (
            <Section title={`Speech pipeline — ${v.speech_aser.clips} real ASER clips, ${v.speech_aser.engine}`}>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="text-left text-xs uppercase tracking-wider text-ink-600"><tr><th className="py-1 pr-3">Level</th><th className="pr-3">Clips</th><th className="pr-3">Agreement with examiner</th><th className="pr-3">Precision</th><th className="pr-3">Recall</th><th>Empty transcripts</th></tr></thead>
                  <tbody>
              ...
              {v.speech_aser.phoneme_layer && Object.keys(v.speech_aser.phoneme_layer.per_level).length > 0 && (
                <>
                  ...
                  <p className="mt-2 text-xs text-ink-600">{v.speech_aser.phoneme_layer.note} Lexora therefore counts an item Whisper rejected as correct when the sounds matched, and uses the phoneme layer to flag mispronounced words that Whisper would silently repair (measured on a real test recording: “he as a blue shit” → Whisper “he has a blue shirt”).</p>
                </>
              )}
              <p className="mt-3 text-sm text-ink-700">{v.speech_aser.interpretation}</p>
              <p className="mt-1 text-xs text-ink-600">{v.speech_aser.sampling}. Median rate of correctly read sentences in this sample: {v.speech_aser.sentence_wpm_examiner_correct.median} wpm.</p>
            </Section>
          )}
          ...
          {v.speech_nnces_pilot && (
            <Section title={`NNCES pilot — ${v.speech_nnces_pilot.files} Telugu-L1 children’s recordings (audio only)`}>
              ...
              <p className="mt-3 text-xs text-ink-600">{v.speech_nnces_pilot.note}</p>
            </Section>
          )}

          {v.rello_methodology && (
            <Section title="Method check — XGBoost + SHAP on Rello et al. (Spanish, diagnosed labels)">
              <p className="text-sm text-ink-700">{v.rello_methodology.n_desktop.toLocaleString()} children ({v.rello_methodology.positives_desktop} with a dyslexia diagnosis), 5-fold stratified cross-validation:</p>
              ...
              <p className="mt-3 text-sm text-ink-700">{v.rello_methodology.interpretation}</p>
              <p className="mt-2 rounded-xl bg-sun-100/60 p-3 text-xs text-ink-700">{v.rello_methodology.transfer_note}</p>
            </Section>
          )}
        </>
      )}

      <Section title="Data sources" aside={<Database className="h-5 w-5 text-ink-600" />}>
    ...
    return (
      <AppShell>
        <div className="mb-7">
          ...
          <h1 className="font-display text-[26px] font-bold leading-tight text-ink-900 md:text-[34px]">Datasets, models and validation</h1>
        </div>
        {body}
      </AppShell>
    )
  ...
  return (
    <PublicShell>
      <section className="band-sand relative px-5 pb-20 pt-14 text-center md:px-7">
        <FlaskConical className="mx-auto h-8 w-8 text-terra-500" aria-hidden />
        <h1 className="mt-4 font-display text-[26px] font-bold leading-tight text-ink-900 md:text-[36px]">
          Datasets, models and validation
        </h1>
        <p className="mx-auto mt-3 max-w-[620px] text-[15px] font-medium leading-relaxed text-ink-700">
          What Lexora was built from, what was measured, and what those numbers do and do not show.
        </p>
        <Wave fill="#FDF9F2" />
      </section>
      <div className="mx-auto max-w-5xl px-5 pb-20 pt-10 md:px-7">
        <Link to="/how-it-works" className="mb-5 inline-block text-sm font-bold text-terra-500 hover:text-terra-600">← Back to how it works</Link>
        {body}
      </div>
    </PublicShell>
  )
...
function Metric({ label, value, hint }: { label: string; value: string; hint?: string }) {
  return <div className="rounded-xl bg-cream-200 p-4"><p className="text-[11px] font-bold uppercase tracking-[0.14em] text-ink-500">{label}</p><p className="mt-1 font-display text-xl font-bold text-ink-900">{value}</p>{hint && <p className="mt-0.5 text-xs font-medium text-ink-500">{hint}</p>}</div>
}
