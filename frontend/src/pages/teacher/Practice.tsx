        ...
        <div className="card p-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <div><h2 className="text-xl font-bold">{shown.title}</h2><p className="text-sm text-navy-500">{shown.content.instructions}</p></div>
            </div>
          ...
          <p className="mt-3 text-xs text-navy-500">Targets: {shown.target_skills.map((s) => SKILL_LABEL[s] ?? s).join(' · ')} · {shown.attempt_count} attempt{shown.attempt_count === 1 ? '' : 's'}{shown.best_score !== null && ` · best ${Math.round(shown.best_score * 100)}%`}</p>
          <div className="mt-5 grid gap-2 text-sm md:grid-cols-2">
            {shown.content.word_choice.map((w, i) => <div key={i} className="rounded-xl bg-cream-100 p-3"><span className="font-bold">{w.answer}</span> <span className="text-navy-500">options: {w.options.join(', ')}</span></div>)}
            {shown.content.spelling.map((w, i) => <div key={i} className="rounded-xl bg-cream-100 p-3"><span className="font-bold">{w.word}</span> <span className="text-navy-500">{w.hint}</span></div>)}
            ...
            {shown.content.questions.map((q, i) => <div key={i} className="rounded-xl bg-cream-100 p-3"><span className="font-bold">{q.question}</span> <span className="text-navy-500">→ {q.answer} (options: {q.options.join(', ')})</span></div>)}
          </div>
        </div>
      ) : (
